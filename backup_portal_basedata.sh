...
if [ -z $1 ]; then
  ...
  BASETABLES='(agreement_categories|app_status|company_application_status|company_role_descriptions|company_roles|company_status|company_user_status|consent_status|countries|document_types|identity_provider_categories|invitation_status|languages|use_cases)'
  if [ -z "$DEVENV_IP" ]; then

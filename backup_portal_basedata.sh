...
if [ -z $1 ]; then
  ...
  BASETABLES='(agreement_categories|app_subscription_statuses|app_statuses|audit_operation|company_application_statuses|company_role_descriptions|company_roles|company_service_account_statuses|company_statuses|company_user_statuses|connector_statuses|connector_types|consent_statuses|countries|document_types|document_status|identity_provider_categories|invitation_statuses|languages|notification_type|use_cases|audit_company_users_CPLP_1254_db_audit|audit_company_assigned_apps_cplp_1253_company_assigned_app)'
  if [ -z "$DEVENV_IP" ]; then

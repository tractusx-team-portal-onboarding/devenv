...
if [ -z $1 ]; then
  ...
  BASETABLES='(agreement_categories|app_subscription_statuses|app_statuses|app_type|audit_operation|company_application_statuses|company_role_descriptions|company_roles|company_service_account_statuses|company_statuses|company_user_statuses|connector_statuses|connector_types|consent_statuses|countries|document_types|document_status|identity_provider_categories|invitation_statuses|languages|notification_type|use_cases)'
  if [ -z "$DEVENV_IP" ]; then

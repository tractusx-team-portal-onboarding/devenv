...
if [ -z $1 ]; then
  ...
  TABLES='(addresses|offer_assigned_documents|app_languages|agreements|agreement_assigned_offer_types|agreement_assigned_offers|agreement_assigned_company_roles|companies|company_identity_providers|document_templates|identity_providers|iam_identity_providers|agreement_assigned_document_templates|iam_clients|user_roles|user_role_descriptions|app_instances|offer_licenses|offer_assigned_licenses|app_assigned_use_cases|offer_descriptions|company_applications|offer_subscriptions|app_subscription_details|company_assigned_roles|company_assigned_use_cases|company_users|company_user_assigned_app_favourites|company_user_assigned_business_partners|company_user_assigned_roles|documents|consents|iam_users|invitations|connectors|company_service_accounts|notifications|offers|offer_detail_images|offer_tags|consent_assigned_offer_subscriptions)'
  if [ -z "$DEVENV_IP" ]; then
